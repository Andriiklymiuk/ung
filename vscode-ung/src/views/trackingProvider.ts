...
export class TrackingProvider
  implements vscode.TreeDataProvider<TrackingTreeItem>
{
  private _onDidChangeTreeData: vscode.EventEmitter<
    TrackingTreeItem | undefined | null | undefined
  ...
  private async getRootItems(): Promise<TrackingTreeItem[]> {
    ...
    const todaySessions = this.cachedSessions.filter((s) =>
    ...
    const todayBillable = todaySessions
      .filter((s) => s.billable)
    ...
    const weekSessions = this.cachedSessions.filter(
    ...
    const clientNames = [
      ...new Set(this.cachedSessions.map((s) => s.client).filter((c) => c)),
    ];
  ...
  private async getItemsForSection(
    ...
    if (section === 'clients') {
      ...
      return clientNames.map((client) => {
        const clientSessions = this.cachedSessions.filter(
          (s) => s.client === client
        );
        const totalMinutes = clientSessions.reduce(
          (sum, s) => sum + s.durationMinutes,
          0
        );
  ...
  private async loadData(): Promise<void> {
    await Promise.all([this.loadActiveSession(), this.loadSessions()]);
  }
  ...
  private async loadSessions(): Promise<void> {
    ...
    try {
      ...
      vscode.window.showErrorMessage(
        `Failed to load tracking sessions: ${error}`
      );
      this.cachedSessions = [];
    }
  }
