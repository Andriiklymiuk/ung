...
type ContractTreeItem = ContractSectionItem | ContractSummaryItem | ContractItem | ContractActionItem;
...
export class ContractItem extends vscode.TreeItem {
    constructor(
        ...
        this.tooltip.appendMarkdown(`- Status: ${active ? '🟢 Active' : '⚪ Inactive'}\n`);
    ...
    private getContractIcon(type: string, active: boolean): vscode.ThemeIcon {
        ...
        switch (typeLower) {
            ...
            case 'milestone':
                return new vscode.ThemeIcon('milestone', new vscode.ThemeColor('charts.orange'));
            default:
 ...
 */
export class ContractProvider implements vscode.TreeDataProvider<ContractTreeItem> {
    ...
    private async getRootItems(): Promise<ContractTreeItem[]> {
        ...
        if (activeContracts.length > 0) {
            ...
            items.push(new ContractSummaryItem(
                'Active Contracts',
        ...
        // Quick actions
        items.push(new ContractActionItem(
            'Create New Contract',
            'add',
            'ung.createContract',
            'charts.blue'
        ...
        const typeGroups = [
            { key: 'hourly', label: 'Hourly Contracts', icon: 'clock', color: 'charts.blue' },
            { key: 'retainer', label: 'Retainers', icon: 'calendar', color: 'charts.purple' },
            { key: 'fixed', label: 'Fixed Price', icon: 'package', color: 'charts.green' },
            { key: 'milestone', label: 'Milestone', icon: 'milestone', color: 'charts.orange' },
        ];

        for (const group of typeGroups) {
            const contracts = activeContracts.filter(c => {
                const typeLower = c.type.toLowerCase();
                return typeLower === group.key ||
                       (group.key === 'fixed' && typeLower === 'fixed_price');
            });

            if (contracts.length > 0) {
                const totalValue = contracts.reduce((sum, c) => sum + c.rateValue, 0);
                items.push(new ContractSectionItem(
                    group.label,
                    group.key,
                    group.icon,
                    group.color,
    ...
    private async getContractsForSection(section: string): Promise<ContractItem[]> {
    ...
    private parseContractOutput(output: string): ParsedContract[] {
        ...
        for (let i = 1; i < lines.length; i++) { // Skip header
            ...
            if (parts.length >= 5) {
                ...
                const type = parts[3];
                const rate = parts[4];
                const active = parts[5]?.toLowerCase() === 'true' ||
                              parts[5]?.toLowerCase() === 'active' ||
                              parts[5]?.toLowerCase() === 'yes';

                // Parse rate value
                const rateMatch = rate.match(/[\d,]+\.?\d*/);
                const rateValue = rateMatch ? parseFloat(rateMatch[0].replace(/,/g, '')) : 0;
