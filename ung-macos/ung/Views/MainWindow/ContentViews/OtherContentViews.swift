...
struct TrackingContent: View {
    ...
    var filteredSessions: [RecentSession] {
        ...
        return appState.recentSessions.filter { $0.project.localizedCaseInsensitiveContains(searchQuery) }
    }
    ...
    private var startTrackingForm: some View {
        AnimatedCard {
            VStack(alignment: .leading, spacing: Design.Spacing.md) {
                ...
                FormField(label: "Project / Task", text: $projectName, placeholder: "What are you working on?")
                ...
                .buttonStyle(DSPrimaryButtonStyle(color: projectName.isEmpty ? .gray : Design.Colors.success))
            ...
            VStack(alignment: .leading, spacing: Design.Spacing.md) {
                ...
                if filteredSessions.isEmpty {
                    AnimatedEmptyState(
                        ...
                        message: searchQuery.isEmpty ? "Start tracking to see your sessions here" : "Try a different search term"
                    )
                    .frame(height: 200)
                } else {
                    ScrollView {
...
struct ClientsContent: View {
    ...
    var body: some View {
        ZStack {
            VStack(spacing: Design.Spacing.lg) {
                ...
                HStack {
                    VStack(alignment: .leading, spacing: Design.Spacing.xxs) {
                        ...
                        Text("\(appState.clients.count) total")
                            .font(Design.Typography.bodySmall)
                            .foregroundColor(Design.Colors.textSecondary)
                    }
                ...
                if filteredClients.isEmpty {
                    ...
                    AnimatedEmptyState(
                        ...
                        message: searchQuery.isEmpty ? "Add your first client to start tracking time and creating invoices" : "Try a different search term",
                        actionTitle: searchQuery.isEmpty ? "Add Client" : nil,
                        action: searchQuery.isEmpty ? { showAddClient = true } : nil
                    )
                    Spacer()
                } else {
                    ScrollView {
            ...
            // Modals
            if showAddClient || showEditClient {
                Color.black.opacity(0.3)
                    .ignoresSafeArea()
                    .onTapGesture {
                        showAddClient = false
            ...
            if showDeleteConfirmation, let client = selectedClient {
                ...
                ConfirmationDialog(
                    title: "Delete Client",
                    message: "Are you sure you want to delete '\(client.name)'? This will also delete associated contracts and invoices.",
                    destructive: true,
                    confirmTitle: "Delete",
                    onConfirm: {
                        Task {
        ...
        .animation(Design.Animation.smooth, value: showAddClient)
    ...
    private var clientFormSheet: some View {
        ActionMenu(title: showEditClient ? "Edit Client" : "Add Client", isPresented: showEditClient ? $showEditClient : $showAddClient) {
...
struct ContractsContent: View {
    ...
    var body: some View {
        ZStack {
            VStack(spacing: Design.Spacing.lg) {
                ...
                HStack {
                    VStack(alignment: .leading, spacing: Design.Spacing.xxs) {
                        Text("Contracts")
                            .font(Design.Typography.displaySmall)
                        Text("\(appState.contracts.count) total")
                            .font(Design.Typography.bodySmall)
                            .foregroundColor(Design.Colors.textSecondary)
                    }
                    ...
                    .buttonStyle(DSPrimaryButtonStyle())
                    .disabled(appState.clients.isEmpty)
                }
                ...
                if appState.clients.isEmpty {
                    ...
                    AnimatedEmptyState(
                        ...
                        message: searchQuery.isEmpty ? "Create contracts to define rates and terms with your clients" : "Try a different search term",
                        actionTitle: searchQuery.isEmpty ? "Create Contract" : nil,
                        action: searchQuery.isEmpty ? { showAddContract = true } : nil
                    )
                    Spacer()
                } else {
                    ScrollView {
            ...
            // Modals
            if showAddContract || showEditContract {
                Color.black.opacity(0.3)
                    .ignoresSafeArea()
                    .onTapGesture {
        ...
        .animation(Design.Animation.smooth, value: showAddContract)
        .animation(Design.Animation.smooth, value: showEditContract)
        .animation(Design.Animation.smooth, value: showDeleteConfirmation)
    }

    private var contractFormSheet: some View {
        ActionMenu(title: showEditContract ? "Edit Contract" : "New Contract", isPresented: showEditContract ? $showEditContract : $showAddContract) {
            VStack(spacing: Design.Spacing.md) {
                FormField(label: "Contract Name", text: $contractName, placeholder: "e.g., Development Services")
                ...
                NumberStepper(label: "Hourly Rate ($)", value: $rate, range: 1...1000, step: 5, format: "$%.0f")
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: Design.Spacing.sm) {
            HStack {
                ...
                if isHovered {
                    HStack(spacing: Design.Spacing.xs) {
                        AnimatedIconButton(icon: "pencil", color: Design.Colors.primary, size: 24, action: onEdit)
                        AnimatedIconButton(icon: "trash", color: Design.Colors.error, size: 24, action: onDelete)
                    }
                    .transition(.scale.combined(with: .opacity))
                }
            }
...
struct InvoicesContent: View {
    ...
    var body: some View {
        ZStack {
            VStack(spacing: Design.Spacing.lg) {
                ...
                if filteredInvoices.isEmpty {
                    ...
                    AnimatedEmptyState(
                        icon: "doc.plaintext.fill",
                        title: searchQuery.isEmpty && filterStatus == "all" ? "No Invoices Yet" : "No Results",
                        message: searchQuery.isEmpty && filterStatus == "all" ? "Create your first invoice from tracked time" : "Try different filters",
                        actionTitle: searchQuery.isEmpty && filterStatus == "all" ? "Create Invoice" : nil,
                        action: searchQuery.isEmpty && filterStatus == "all" ? { showCreateInvoice = true } : nil
                    )
                    Spacer()
                } else {
                    ScrollView {
            ...
            if showCreateInvoice {
                Color.black.opacity(0.3)
                    .ignoresSafeArea()
                    .onTapGesture { showCreateInvoice = false }
        ...
        .animation(Design.Animation.smooth, value: showCreateInvoice)
        .animation(Design.Animation.smooth, value: showDeleteConfirmation)
        .animation(Design.Animation.smooth, value: filterStatus)
    }

    private var createInvoiceSheet: some View {
        ActionMenu(title: "Create Invoice", isPresented: $showCreateInvoice) {
            VStack(spacing: Design.Spacing.md) {
                VStack(alignment: .leading, spacing: Design.Spacing.xxs) {
                    Text("Client")
                        .font(Design.Typography.labelSmall)
            ...
            if isHovered {
                HStack(spacing: Design.Spacing.xs) {
                    if invoice.status.lowercased() != "paid" {
                        AnimatedIconButton(icon: "checkmark.circle", color: Design.Colors.success, size: 28, action: onMarkPaid)
                    }
                    if invoice.status.lowercased() == "pending" {
                        AnimatedIconButton(icon: "paperplane", color: Design.Colors.primary, size: 28, action: onMarkSent)
                    }
                    AnimatedIconButton(icon: "trash", color: Design.Colors.error, size: 28, action: onDelete)
                }
                .transition(.scale.combined(with: .opacity))
...
struct ExpensesContent: View {
    ...
    let categories = ["Software", "Hardware", "Travel", "Meals", "Office", "Marketing", "Education", "Other"]
    ...
    var body: some View {
        ZStack {
            VStack(spacing: Design.Spacing.lg) {
                ...
                HStack {
                    VStack(alignment: .leading, spacing: Design.Spacing.xxs) {
                        Text("Expenses")
                            .font(Design.Typography.displaySmall)
                        Text("Track business expenses for tax deductions")
                            .font(Design.Typography.bodySmall)
                            .foregroundColor(Design.Colors.textSecondary)
                    }
                ...
                if filteredExpenses.isEmpty {
                    ...
                    AnimatedEmptyState(
                        ...
                        message: searchQuery.isEmpty ? "Track your business expenses for tax deductions" : "Try a different search term",
                        actionTitle: searchQuery.isEmpty ? "Log Expense" : nil,
                        action: searchQuery.isEmpty ? { showLogExpense = true } : nil
                    )
                    Spacer()
                } else {
                    ScrollView {
                            ...
                            ForEach(filteredExpenses) { expense in
                                ExpenseRowView(
                                    ...
                                    onEdit: {
                                        ...
                                        expenseAmount = Double(expense.amount.replacingOccurrences(of: "$", with: "").replacingOccurrences(of: ",", with: "")) ?? 0
            ...
            // Modals
            if showLogExpense || showEditExpense {
                Color.black.opacity(0.3)
                    .ignoresSafeArea()
                    .onTapGesture {
        ...
        .animation(Design.Animation.smooth, value: showLogExpense)
        .animation(Design.Animation.smooth, value: showEditExpense)
        .animation(Design.Animation.smooth, value: showDeleteConfirmation)
    }

    private var expenseFormSheet: some View {
        ActionMenu(title: showEditExpense ? "Edit Expense" : "Log Expense", isPresented: showEditExpense ? $showEditExpense : $showLogExpense) {
            VStack(spacing: Design.Spacing.md) {
                FormField(label: "Description", text: $expenseDescription, placeholder: "What did you buy?")

                NumberStepper(label: "Amount", value: $expenseAmount, range: 0...100000, step: 10, format: "$%.2f")
...
struct ReportsContent: View {
    ...
    let sections = [("overview", "Overview"), ("calculator", "Rate Calculator"), ("goals", "Income Goals")]
    ...
    var body: some View {
        VStack(spacing: Design.Spacing.lg) {
            ...
            HStack {
                ...
                AnimatedSegmentedControl(selection: $selectedSection, options: sections)
            }
    ...
    private var overviewContent: some View {
        VStack(spacing: Design.Spacing.lg) {
            ...
            AnimatedEmptyState(
                ...
                message: "Advanced analytics, charts, and exportable reports will be available in a future update"
            )
            Spacer()
        }
    }
                    ...
                    VStack(alignment: .leading, spacing: Design.Spacing.md) {
                        ...
                        HStack(spacing: Design.Spacing.lg) {
                            ...
                            ProjectionItem(
                                ...
                                value: appState.formatCurrency(max(0, savedMonthlyGoal - appState.metrics.monthlyRevenue)),
...
struct SettingsContent: View {
        ...
        ZStack {
            ScrollView {
                ...
                SettingsSection(title: "About", icon: "info.circle") {
                    ...
                    HStack(spacing: Design.Spacing.md) {
                        Link(destination: URL(string: "https://github.com/Andriiklymiuk/ung")!) {
    ...
    private var companyEditorSheet: some View {
        ActionMenu(title: appState.setupStatus.hasCompany ? "Edit Company" : "Create Company", isPresented: $showCompanyEditor) {
            VStack(spacing: Design.Spacing.md) {
...
struct CompanyProfileSection: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: Design.Spacing.md) {
            ...
            .buttonStyle(DSPrimaryButtonStyle(color: hasCompany ? Design.Colors.primary : Design.Colors.success))
        }
        .padding(Design.Spacing.lg)
        .frame(maxWidth: .infinity, alignment: .leading)
        .background(
            RoundedRectangle(cornerRadius: Design.Radius.md)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: Design.Radius.md)
                        .stroke(hasCompany ? Color.clear : Design.Colors.warning.opacity(0.5), lineWidth: hasCompany ? 0 : 2)
                )
                .shadow(
                    color: isHovered ? Design.Shadow.md.color : Design.Shadow.sm.color,
                    radius: isHovered ? Design.Shadow.md.radius : Design.Shadow.sm.radius,
...
extension UTType {
    static var database: UTType {
        UTType(exportedAs: "com.ung.database", conformingTo: .data)
    }
}
