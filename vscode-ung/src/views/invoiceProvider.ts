...
export class InvoiceProvider
  implements vscode.TreeDataProvider<InvoiceTreeItem>
{
  private _onDidChangeTreeData: vscode.EventEmitter<
    InvoiceTreeItem | undefined | null | undefined
  > = new vscode.EventEmitter<InvoiceTreeItem | undefined | null | undefined>();
  ...
  private async getRootItems(): Promise<InvoiceTreeItem[]> {
    ...
    // Summary section
    const totalAmount = this.cachedInvoices.reduce(
      (sum, inv) => sum + inv.amount,
      0
    );
    const unpaidAmount = this.cachedInvoices
      .filter((inv) =>
        ['pending', 'sent', 'overdue'].includes(inv.status.toLowerCase())
      )
      .reduce((sum, inv) => sum + inv.amount, 0);
    const overdueCount = this.cachedInvoices.filter(
      (inv) => inv.status.toLowerCase() === 'overdue'
    ).length;

    if (this.cachedInvoices.length > 0) {
      items.push(
        new InvoiceSummaryItem(
          'Total Invoiced',
