...
export class ErrorHandler {
     ...
     */
    private static getUserFriendlyMessage(error: Error | UngError | string, context?: string): string {
    ...
    static validateInput(value: string | undefined, fieldName: string): string {
        if (!value || value.trim() === '') {
            throw new UngError(ErrorType.VALIDATION_ERROR, `${fieldName} is required`);
        }
        return value.trim();
    }
     ...
     */
    static validateNumber(value: string | undefined, fieldName: string, min?: number, max?: number): number {
        const validatedString = this.validateInput(value, fieldName);
        const num = Number(validatedString);

        if (isNaN(num)) {
            throw new UngError(ErrorType.VALIDATION_ERROR, `${fieldName} must be a valid number`);
        }

        if (min !== undefined && num < min) {
            throw new UngError(ErrorType.VALIDATION_ERROR, `${fieldName} must be at least ${min}`);
        }

        if (max !== undefined && num > max) {
            throw new UngError(ErrorType.VALIDATION_ERROR, `${fieldName} must be at most ${max}`);
        }
