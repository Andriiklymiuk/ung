...
export class BulkCommands extends BaseCommand {
  ...
  async bulkMarkInvoicesPaid(): Promise<void> {
    const invoices = await this.selectInvoices(
        ...
        for (let i = 0; i < items.length; i++) {
          if (token.isCancellationRequested) {
            NotificationManager.warning(
              `${operationName} cancelled. ${completed} of ${items.length} completed.`
            );
            return;
          }
          ...
          try {
            ...
            ErrorHandler.logWarning(
              `${operationName} failed for ${item.label}: ${error}`
            );
          }
        }
      }
   ...
   */
  private async selectInvoices(
    title: string,
    statusFilter?: string
  ): Promise<BulkItem[] | undefined> {
