      ...
      async () => {
        ...
        if (result.success) {
          ...
          vscode.window.showErrorMessage(
            `Failed to create company: ${result.error}`
          );
        }
      }
    );
  ...
  async editCompany(companyId?: number): Promise<void> {
    ...
    const editField = await vscode.window.showQuickPick(
      [
        ...
        {
          label: '$(credit-card) Edit Bank Name',
          field: 'bankName',
          value: company.bankName || '',
        },
        {
          label: '$(key) Edit Bank Account',
          field: 'bankAccount',
          value: company.bankAccount || '',
        },
        {
          label: '$(globe) Edit Bank SWIFT',
          field: 'bankSwift',
    ...
    if (editField.field === 'all') {
      ...
      const fields = [
        ...
        { key: 'taxId', prompt: 'Tax ID', current: company.taxId || '' },
        {
          key: 'bankName',
          prompt: 'Bank Name',
          current: company.bankName || '',
        },
        {
          key: 'bankAccount',
          prompt: 'Bank Account',
          current: company.bankAccount || '',
        },
        {
          key: 'bankSwift',
          prompt: 'Bank SWIFT/BIC',
          current: company.bankSwift || '',
        },
      ];
      ...
      for (const field of fields) {
        const newValue = await vscode.window.showInputBox({
          ...
          validateInput: field.required
            ? field.isEmail
              ? (v) => (v?.includes('@') ? null : 'Valid email required')
              : (v) => (v ? null : `${field.prompt} is required`)
            : undefined,
        });
      ...
      // Edit single field
      const isRequired =
        editField.field === 'name' || editField.field === 'email';
    ...
    await vscode.window.withProgress(
      ...
      async () => {
        ...
        if (editResult.success) {
          ...
          vscode.window.showErrorMessage(
            `Failed to update company: ${editResult.error}`
          );
        }
      }
    );
    ...
    await vscode.window.withProgress(
      ...
      async () => {
        ...
        if (result.success && result.stdout) {
          ...
          vscode.window.showErrorMessage(
            `Failed to list companies: ${result.error}`
          );
        }
      }
    );
