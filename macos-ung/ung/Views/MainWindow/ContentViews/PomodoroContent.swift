...
struct PomodoroContent: View {
  ...
  private var timerSection: some View {
    VStack(spacing: 32) {
      ...
      // Control buttons - fixed height container to prevent jumping
      HStack(spacing: 16) {
        if appState.pomodoroState.isActive {
          // Pause/Resume button
          Button(action: {
          ...
          // Start button - same total width as active buttons
          Button(action: { appState.startPomodoro() }) {
            ...
            .frame(width: 292)  // Same as 100 + 80 + 80 + 16*2 spacing
            .padding(.vertical, 14)
            .background(
      ...
      .frame(minWidth: 292, minHeight: 48)  // Fixed size to prevent jumping
    }
  }
  ...
  private var statsCard: some View {
    VStack(alignment: .leading, spacing: 16) {
      ...
      HStack(spacing: 24) {
        ...
        StatItem(
          value:
            "\(appState.pomodoroState.sessionsCompleted * appState.pomodoroState.workMinutes)m",
          label: "Focus Time",
        ...
        StatItem(
          value:
            "\(appState.pomodoroState.sessionsCompleted / appState.pomodoroState.sessionsUntilLongBreak)",
          label: "Cycles",
