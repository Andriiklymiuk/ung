...
export class ContractCommands {
    ...
    async editContract(contractId?: number): Promise<void> {
        ...
        if (contracts.length === 0) {
            vscode.window.showInformationMessage('No contracts found. Create one first.');
            return;
        }
        ...
        const priceMatch = contract.ratePrice.match(/(\d+(?:\.\d+)?)\s*(?:USD|EUR|GBP|CHF|PLN)/i);
        ...
        const editField = await vscode.window.showQuickPick([
            ...
            { label: '$(symbol-number) Edit Hourly Rate', field: 'rate', value: currentRate?.toString() || '' },
            { label: '$(credit-card) Edit Fixed Price', field: 'price', value: currentPrice?.toString() || '' },
            { label: '$(globe) Edit Currency', field: 'currency', value: currentCurrency },
            { label: '$(circle-filled) Toggle Active Status', field: 'active', value: contract.active ? 'true' : 'false' },
        ...
        if (editField.field === 'all') {
            ...
            const newRate = await vscode.window.showInputBox({
                ...
                validateInput: v => {
                    ...
                    return isNaN(num) || num < 0 ? 'Enter a valid positive number' : null;
                }
            });
            if (newRate === undefined) return;
            if (newRate && parseFloat(newRate) !== currentRate) updates.rate = parseFloat(newRate);
            ...
            if (newPrice && parseFloat(newPrice) !== currentPrice) updates.price = parseFloat(newPrice);
            ...
            if (newRate === undefined) return;
            if (parseFloat(newRate) !== currentRate) updates.rate = parseFloat(newRate);
            ...
            if (newPrice === undefined) return;
            if (parseFloat(newPrice) !== currentPrice) updates.price = parseFloat(newPrice);
        ...
        await vscode.window.withProgress({
            ...
            if (editResult.success) {
                vscode.window.showInformationMessage(`Contract "${contract!.name}" updated successfully!`);
                if (this.refreshCallback) {
                    this.refreshCallback();
                }
            } else {
                vscode.window.showErrorMessage(`Failed to update contract: ${editResult.error}`);
            }
        });
    }
